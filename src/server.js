...
function handleTileClick(event) {
    // Finde heraus, ob der Edit-Button oder die Kachel selbst geklickt wurde
    const clickedEditButton = event.target.closest('.edit-event-btn');
    const clickedTile = event.target.closest('.event-kachel');
    const clickedLink = event.target.closest('a'); // Verhindern, dass Klicks auf Links die Kachel togglen

    // 1. Klick auf Edit-Button? -> Nur Passwortabfrage/Modal öffnen
    if (clickedEditButton) {
        console.log('Edit button clicked');
        event.stopPropagation(); // Verhindert, dass der Klick zur Kachel "durchblubbert"
        const rowNum = parseInt(clickedEditButton.getAttribute('data-row'), 10);
        if (rowNum) {
            promptForEditPassword(rowNum); // Bestehende Funktion aufrufen
        }
        return; // Wichtig: Hier beenden, damit Kachel nicht toggled
    }

    // 2. Klick auf einen Link innerhalb der Kachel? -> Nichts tun (Link folgen lassen)
     if (clickedLink) {
         console.log('Link clicked, not toggling tile.');
         // event.stopPropagation(); // Normalerweise nicht nötig, da wir eh returnen
         return;
     }


    // 3. Klick auf die Kachel (aber nicht auf den Edit-Button oder Link)? -> Auf-/Zuklappen
    if (clickedTile) {
        console.log('Tile area clicked:', clickedTile.dataset.rowNum);

        // Finde die aktuell geöffnete Kachel (falls vorhanden)
        const currentlyExpanded = listElement.querySelector('.event-kachel.expanded');

        // Wenn eine andere Kachel offen war UND es nicht die ist, die wir gerade geklickt haben -> Schliessen
        if (currentlyExpanded && currentlyExpanded !== clickedTile) {
            currentlyExpanded.classList.remove('expanded');
        }

        // Die geklickte Kachel auf-/zuklappen (toggle)
